...
export async function runCreative({ offer, context }) {
  ...
  const creatives = angles.map((angle) => {
    ...
    return {
      ...
      preferred_packs: assets.filter((a) => {
        if (angle.id === 'travel') return /travel/i.test(a);
        if (angle.id === 'services') return /service|subscription/i.test(a);
        if (angle.id === 'premium') return /premium|banners-all/i.test(a);
        return true;
      }),
    };
  });
  ...
  const briefPathHint =
    'creatives/rsya/ — используй generate_banners_lg2026.py (или v2/v3) и готовые zip/mp4';
