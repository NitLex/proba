...
2. **Добавить счётчик**:
   ...
   - Часовой пояс: Москва
3. Создай цели:
   - **JavaScript-событие** название `soft_lead`, идентификатор `soft_lead`
     (в кабинете цель уже есть: **ID 246** — его ставим в Direct PriorityGoals)
   - Позже hard: оффлайн-конверсия / постбек (выдача карты) — когда пойдёт объём
4. Скопируй **номер счётчика** (цифры в шапке счётчика, обычно 7–8 знаков) и при желании **ID целей**.

Уже зафиксировано:
- soft goal name / JS: `soft_lead`
- soft goal id (Direct): `246`
