...
### Phase A — скорость байера (сделано)
- Dashboard: период, Today/7d/30d, Profit/ROI/EPC/CPA, топ, последние конверсии, простые алерты  
- Stats: колонка CPA  
- Logs: даты, поиск, refresh, лимит  

### Phase B — доверие к цифрам (сделано)
- `path_id` / `rule_id` на клике → отчёты **По путям** / **По правилам**  
- Исходящий `postback_url` источника при конверсии (макросы `{clickid}{payout}{status}{txid}…`)  
- Telegram-алерты: 0 конверсий / ROI ниже порога / кампания без лидов (`TELEGRAM_BOT_TOKEN` + chat_id в профиле)  

### Phase C — дальше
- Алерты на Discord/Email  
- By-landing отчёт  
- Командные роли  

### Phase D — плагины
- API источников, GEO-карта, глубокий antifraud, прогнозирование  
